...
```admonish note title="Взгляд вперёд: почему String не для продакшена"
Вы могли заметить, что во всех примерах мы используем `String` для текста. Но `String = [Char]` — связный список символов! Это крайне неэффективно:

- Каждый символ — отдельная аллокация (около 40 байт на символ вместо 1–4)
- Конкатенация, поиск, длина — O(n) с большими константами
- Юникод работает, но медленно

В [следующей главе](chapter06.md) мы перейдём на **Text** — компактное представление строк в UTF-16, где большинство операций работают за O(1) или близко к этому. Мы также освоим `Map` и `Set` — эффективные структуры данных для хранения задач трекера.

**Правило для production-кода:** ВСЕГДА используйте `Text` вместо `String`. `String` — только для учебных примеров и legacy API.
```
