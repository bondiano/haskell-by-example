...
```admonish warning title="Опасность: парциальные функции"
Некоторые функции из Prelude **парциальные** — они падают с ошибкой на определённых входах:

```haskell
head :: [a] -> a           -- падает на пустом списке
tail :: [a] -> [a]         -- падает на пустом списке
(!!) :: [a] -> Int -> a    -- падает на некорректном индексе
read :: Read a => String -> a  -- падает при ошибке парсинга

> head []
*** Exception: Prelude.head: empty list
```

**Правило:** Избегайте парциальных функций в production-коде. Используйте безопасные альтернативы:

- `listToMaybe :: [a] -> Maybe a` вместо `head`
- Паттерн-матчинг `(x:xs)` вместо `head`/`tail`
- `!?` из `safe` пакета вместо `(!!)`
- `readMaybe :: Read a => String -> Maybe a` вместо `read` (см. главу 8)

Если вы уверены, что список непустой — используйте паттерн-матчинг, а не `head`. Это явно документирует инвариант.
```
