...
**Изоморфизм** — это линза, которая фокусирует *всю* структуру, а не её часть. Два типа изоморфны, если между ними существует биекция (`to . from == id` и `from . to == id`):
...
```text
> view celsiusFahrenheit 100
212.0

> view (from celsiusFahrenheit) 212
100.0
```

`from` — переворачивает `Iso`, позволяя смотреть в обратном направлении. Это работает *только* для `Iso`, не для обычных линз (у линзы направление фиксировано).

Другие стандартные изоморфизмы:

```haskell
-- curried / uncurried
> view curried (\(a, b) -> a + b) 1 2
3

-- newtypes
> view (coerced :: Iso' (Sum Int) Int) (Sum 42)
42
...
```admonish tip title="Iso и рефакторинг"
Если рефакторинг меняет представление данных, но не теряет информации — вы создаёте
изоморфизм. `Iso` позволяет формализовать это и использовать через композицию с другими
оптиками. Подробнее об изоморфизмах — в главе 19.
```

## Заметка: Profunctor encoding

Van Laarhoven encoding, который мы реализовали выше, — не единственный способ определить оптики. Существует эквивалентный **profunctor encoding**:
...
Profunctor — тип, который контравариантен по первому аргументу и ковариантен по второму (`dimap`). Из таблицы видно, что `Iso` — самый общий (только `Profunctor`), а `Lens` и `Prism` добавляют ограничения (`Strong`, `Choice`).

Мы разберём `Profunctor` в главе 19, где эта связь оптик с теорией категорий станет яснее.
...
- Разобрали `Iso` — изоморфизмы как оптики с биекцией.
- Реализовали собственные линзы через кодировку ван Лаарховена.
- Узнали о profunctor encoding — альтернативном представлении оптик (подробнее в главе 19).
